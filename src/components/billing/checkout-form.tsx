...
export function CheckoutForm({ planId, billingCycle, onSuccess, onCancel }: CheckoutFormProps) {
  ...
  const inputClass =
    "w-full rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-colors"
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" required placeholder="MM" maxLength={2} value={form.expiryMonth}
                onChange={(e) => set("expiryMonth", e.target.value.replace(/\D/g, ""))} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">Ano</label>
              <input type="text" required placeholder="AAAA" maxLength={4} value={form.expiryYear}
                onChange={(e) => set("expiryYear", e.target.value.replace(/\D/g, ""))} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">CVV</label>
              <input type="text" required placeholder="123" maxLength={4} value={form.ccv}
                onChange={(e) => set("ccv", e.target.value.replace(/\D/g, ""))} className={inputClass} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" required placeholder="00000-000" maxLength={9} value={form.postalCode}
                onChange={(e) => set("postalCode", e.target.value)} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">Número</label>
              <input type="text" required placeholder="100" value={form.addressNumber}
                onChange={(e) => set("addressNumber", e.target.value)} className={inputClass} />
            </div>
          </div>
        </>
      )}
