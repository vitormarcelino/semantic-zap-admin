...
function parseInline(line: string): React.ReactNode {
  ...
  return tokens.map((tok, i) => {
    ...
    if (tok.type === "mono") return <code key={i} className="rounded bg-muted px-1 font-mono text-[0.85em]">{tok.text}</code>
  })
}
...
function SenderBadge({ sentBy }: { sentBy: string | null }) {
  ...
  if (sentBy === "bot") {
    return (
      <span className="inline-block rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] font-medium text-muted-foreground">
        Bot
      </span>
    )
  }
  if (sentBy.startsWith("operator:")) {
    return (
      <span className="inline-block rounded px-1.5 py-0.5 font-mono text-[10px] font-medium bg-blue-500/20 text-blue-600 dark:text-blue-400">
        Operator
      </span>
    )
  }
    ...
    <div className={cn("flex flex-col gap-1", isOutbound ? "items-end self-end" : "items-start self-start")}>
      ...
      <div className={cn("flex items-center gap-1.5 px-1", isOutbound ? "flex-row-reverse" : "flex-row")}>
        ...
        {message.mediaType === "voice" && (
          <Mic size={12} strokeWidth={1.5} strokeLinecap="round" className="text-muted-foreground/60" />
        )}
